...
int end_player_cmp(struct end_player* const* a, struct end_player* const* b) {
  ...
  return global_api_get()->user_cmp(&(x->user), &(y->user));
}
...
struct end_player* end_player_init(unsigned long uuid) {
  ...
  struct user* disc = api->user_get(uuid);
  if (disc == NULL) {
    log_error(api, "Could not initialize player %zi", uuid);
    return NULL;
  }
  if (disc->uuid != uuid) {
    log_error(api, "Given UUID does not match returned UUID (%zi)", disc->uuid);
    return NULL;
  }
  ...
  if (api->registry_add(end_regman_get_player(), &player) == NULL) {
    log_error(api, "Could not initialize player %zi", uuid);
    free(player);
  ...
  log_info(api, "Initializing player %s (%zi)", player->user->username, uuid);
  pthread_rwlock_unlock(&lock);
  return player;
}
...
struct end_player* end_player_get(unsigned long uuid) {
  ...
  struct end_player** ret_ptr = api->registry_ktov(end_regman_get_player(), &key);
  if (ret_ptr == NULL) {
    pthread_rwlock_unlock(&lock);
